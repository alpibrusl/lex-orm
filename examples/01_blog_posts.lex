...
fn identity_decode(j :: jv.Json) -> Result[jv.Json, s.Errors] { Ok(j) }
...
fn main() -> Str {
  ...
  let select_plan := q.build_select(select_q)
  ...
  str.join([
    ...
    str.concat("params: ", str.from_int(list.len(select_plan.params))),
    "",
    str.concat("INSERT: ", insert_plan.sql),
    str.concat("params: ", str.from_int(list.len(insert_plan.params))),
    "",
    str.concat("UPDATE: ", update_plan.sql),
    str.concat("params: ", str.from_int(list.len(update_plan.params))),
    "",
    str.concat("DELETE: ", delete_plan.sql),
    str.concat("params: ", str.from_int(list.len(delete_plan.params))),
  ], "\n")
}
