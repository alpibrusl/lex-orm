...
fn select_multi_where_uses_and() -> Result[Unit, Str] {
  let sel := q.build_select(q.where_clause(q.where_clause(q.select(post_repo()), p.eq("id", PInt(5))), p.eq("slug", PStr("hello"))))
...
fn update_has_set() -> Result[Unit, Str] {
  let upd := q.build_update(q.where_update(q.set_col(q.update(post_repo()), "title", PStr("New")), p.eq("id", PInt(7))))
...
fn delete_with_where() -> Result[Unit, Str] {
  let del := q.build_delete(q.where_delete(q.delete_from(post_repo()), p.eq("id", PInt(3))))
